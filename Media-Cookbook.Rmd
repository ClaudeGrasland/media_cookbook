...
```{r}
extract_tags(qd = qd, # the corpus of interest
             ...
             keep_apos  = FALSE, # keep apostrophs ? 
             tolow = FALSE # Transform in lower case ?
             )
{
# Tokenize text
toks<-tokens(qd)
if(keep_apos == FALSE) {  toks<- tokens_split(toks, "'")}  
if(tolow ==TRUE)       { toks <- tolower(toks)}  
...
ctrl<-ctrl[ntoken(ctrl$label)>1,]
labels <-ctrl$label
if(keep_apos == FALSE) {labels<- gsub("'"," ",labels)}  
if(tolow ==TRUE)       {labels <- tolower(labels)}  
