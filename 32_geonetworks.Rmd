...
We load the hypercube describing the association between regions and states in the same titles of news
...
nodesi <- left_join(nodesi,tot) %>% mutate(value = sqrt(2+10 *size/max(size)))
...
tot$code<-as.factor(tot$code)
nodesj <- left_join(nodesj,tot) %>% mutate(value = sqrt(1+10 *size/max(size)))
